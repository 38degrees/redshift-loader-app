...
sidekiq_options retry: 1,
  ...
  def perform(table_id, lock_name)
    ...
    logger.info "[perform] START for table_id=#{@table_id}, lock_name=#{@lock_name}"
  
    t = Table.find(@table_id)
    rows_copied = t.copy_now
  
    run_again = (rows_copied >= t.import_row_limit)
    logger.info "[perform] FINISH for table_id=#{@table_id}, lock_name=#{@lock_name}, run_again=#{run_again}"
